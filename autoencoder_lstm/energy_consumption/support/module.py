...
class EnergyConsumptionModule(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, lstm_hidden_size=50, lstm_num_layers=2):
        super().__init__()

        # CNN branch using Sequential
        self.cnn_branch = torch.nn.Sequential(
            torch.nn.Conv1d(in_channels=input_dim, out_channels=64, kernel_size=5, padding=2),
        ...
        fusion_size = self.cnn_output_size + lstm_hidden_size
